...
class Channel (object):
    ...
    def invoke_subsystem(self, subsystem):
        ...
        m.add_byte(byt(MSG_CHANNEL_REQUEST))
        m.add_int(self.remote_chanid)
        m.add_bytes(b'subsystem')
        m.add_boolean(True)
        m.add_bytes(subsystem)
        self._event_pending()
        self.transport._send_user_message(m)
        self._wait_for_event()
    ...
    def request_x11(self, screen_number=0, auth_protocol=None, auth_cookie=None,
        ...
        m.add_byte(byt(MSG_CHANNEL_REQUEST))
        m.add_int(self.remote_chanid)
        m.add_bytes(b'x11-req')
        m.add_boolean(True)
        m.add_boolean(single_connection)
        m.add_bytes(auth_protocol)
        m.add_bytes(auth_cookie)
    ...
    def recv(self, nbytes):
        ...
        if ack > 0:
            m = Message()
            m.add_byte(byt(MSG_CHANNEL_WINDOW_ADJUST))
            m.add_int(self.remote_chanid)
            m.add_int(ack)
            self.transport._send_user_message(m)
    ...
    def send(self, s):
        ...
        try:
            ...
            if size == 0:
                ...
                return 0
            m = Message()
            m.add_byte(byt(MSG_CHANNEL_DATA))
            m.add_int(self.remote_chanid)
            m.add_bytes(s[:size])
        finally:
            self.lock.release()
    ...
    def send_stderr(self, s):
        ...
        try:
            ...
            if size == 0:
                ...
                return 0
            m = Message()
            m.add_byte(byt(MSG_CHANNEL_EXTENDED_DATA))
            m.add_int(self.remote_chanid)
            m.add_int(1)
            m.add_bytes(s[:size])
        finally:
    ...
    def _handle_request(self, m):
        ...
        if want_reply:
            m = Message()
            if ok:
                m.add_byte(byt(MSG_CHANNEL_SUCCESS))
            else:
                m.add_byte(byt(MSG_CHANNEL_FAILURE))
            m.add_int(self.remote_chanid)
            self.transport._send_user_message(m)
    ...
    def _send_eof(self):
        ...
        if self.eof_sent:
            return None
        m = Message()
        m.add_byte(byt(MSG_CHANNEL_EOF))
        m.add_int(self.remote_chanid)
        self.eof_sent = True
    ...
    def _close_internal(self):
        ...
        m1 = self._send_eof()
        m2 = Message()
        m2.add_byte(byt(MSG_CHANNEL_CLOSE))
